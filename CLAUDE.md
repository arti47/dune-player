...
- [~] **Phase 2 — Core Tracker** *(core in; skill/drive/focus/talent editing + portrait deferred to the advancement loop, Phase 4)*
  ...
  - [x] **Per-character Markdown export/import** on the live sheet → `store.characterToMarkdown`/`characterFromMarkdown`/`importCharacterMarkdown`: "⬇ Export sheet (.md)" downloads a readable `<name>.md` (skills/drives/statements/focuses/talents/traits/assets/ambition/notes) carrying a trailing HTML-comment **data island** for lossless round-trip; "Import (.md)" in the Characters card adds it as a **new character** (fresh id), rejecting files with no data island *(2026-07-09 — npm test round-trip + reject + fresh-id; browser-verified export→import adds a copy, 0 console errors)*
