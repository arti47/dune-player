...
- [~] **Phase 2 — Core Tracker** *(core in; skill/drive/focus/talent editing + portrait deferred to the advancement loop, Phase 4)*
  ...
  - [x] **Per-character Markdown export/import** on the live sheet → `store.characterToMarkdown`/`characterFromMarkdown`: "⬇ Export sheet (.md)" downloads a readable `<name>.md` (skills/drives/statements/focuses/talents/traits/assets/ambition/notes) carrying a trailing HTML-comment **data island** for lossless round-trip; "Import (.md)" in the Characters card parses the file then opens a **target-choice dialog** — add as a **new character** (fresh id) or **replace an existing one** (picked from the roster, confirm-gated). Files with no data island are rejected *(2026-07-09 — npm test round-trip + reject + fresh-id; browser-verified export→import New adds a copy, Replace overwrites in place, Cancel no-ops, 0 console errors)*
