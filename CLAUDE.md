...
- [~] **Phase 3 — Dice Engine** *(core skill-test roller in; opposed/assist/talent-automation/Architect/citations deferred to later Phase-3 passes)*
  ...
  - [~] Talent-embedded automation (§3.9), driven by the machine-readable `auto` descriptors: **Voice** (`buyAutoSuccessesWithThreat` — 1–3 auto-successes for 1–3 Threat on a matching-skill test) and **Mentat Discipline** (`autoOneDie` — one die auto-counts as 1) surface in the roll dialog when the character has the talent and the chosen skill matches. *(Prana-Bindu re-roll, Calculated Prediction, etc. still to wire)*
