...
| `combat.js` | Scene/adventure lifecycle engine (§3.17: `endScene`/`endAdventure`/`renderLifecycle`), generic extended-task tracker (`scoreExtendedTask`/`renderTasks`), guided defeat + recovery (`renderDefeat`), and the local conflict helper (§3.12: `renderConflict` + pure `startConflict`/`takeTurn`/`nextRound`/`opposingSide` — zones, sides, initiative passing, per-combatant defeat tracks, NPC drop-in). Phase 5 adds shared/two-way pending-contest sync. |
| `gm.js` | GM dashboard (gated by `Settings.gmScreen`): Threat/Momentum pools, party peek sheets, searchable NPC compendium, §3.16 rollable tables (`storyHookIndex`/`rowForRoll` + Story Hook & enemy generators), and the greatGame-gated Planet generator (The Great Game d20 tables). |
...
| `content.js` | The effective content set: core data + any **enabled expansion** crunch (§4/§8). `allTalents`/`findTalent`/`allFactionTemplates`/`allArchetypes`/`focusExamplesFor` + `allDrives`/`driveName`/`driveStatementExamplesFor` (alternative drives) — every talent/faction/focus/drive surface reads through here so expansion content appears only when its toggle is on. No rules data lives here. |
