...
| `combat.js` | Scene/adventure lifecycle engine (§3.17: `endScene`/`endAdventure`/`renderLifecycle`), generic extended-task tracker (`scoreExtendedTask`/`renderTasks`), guided defeat + recovery (`renderDefeat`) — all done. To come: the shared conflict tracker (5 conflict types, zones, initiative-passing [acted-this-round flags, Keep-the-Initiative not-twice-in-a-row, last-actor-picks-opener], pending-contest coordination [defender rolls first]). |
...
- [~] **Phase 4 — In-Play Systems** *(lifecycle + advancement + extended-task tracker + guided defeat/recovery in; local conflict helper to follow)*
