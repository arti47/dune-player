...
- [~] **Phase 6 — Conditional surfaces** *(GM screen in; expansion extraction blocked on the notebook source)*
  ...
  - [ ] Expansion extraction + toggles (§4): ledger **T33–T37** per book — committed: Sand and Dust, The Great Game, Power and Pawns; stretch: Masters of Dune, Fall of the Imperium. **Blocked:** each book's crunch must be extracted from the owner's NotebookLM source (per §2 — never fill from memory); the toggle architecture (`settings.js`, gated data files) is ready and awaits the owner supplying book text
- [ ] **Hardening**
