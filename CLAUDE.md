...
| `combat.js` | Scene/adventure lifecycle engine (§3.17: `endScene`/`endAdventure`/`renderLifecycle` — done) + (to come) shared conflict tracker: 5 conflict types, zones, initiative-passing (acted-this-round flags, Keep-the-Initiative not-twice-in-a-row, last-actor-picks-opener), pending-contest coordination (defender rolls first), defeat/Resist Defeat/lasting-defeat flow, generic extended-task tracker. |
...
- [~] **Phase 4 — In-Play Systems** *(lifecycle engine + advancement loop in; defeat/extended-task/recovery/conflict to follow)*
